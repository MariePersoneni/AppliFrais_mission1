...
if($existeFraisKm) {
    ...
    $w = $pdf->GetStringWidth(utf8_decode('Frais kilométriques'));
    $pdf->SetX(0);
    $pdf->Cell(PDF::LARGEUR_PAGE, 6, utf8_decode('Frais kilométriques'), 0, 1, 'C');
    $colonnesFraisKm = array(
         utf8_decode('Type de véhicule'),
         utf8_decode('Kilométres parcourus'),
        'Montant unitaire',
        'Total'
    );
...
if ($idComptable <> '') {
    ...
    $pdf->SetFont('Times', '', 11);
    $pdf->Cell(0,8, utf8_decode('Fait à Paris, le ') . $dateValidation,0,2);
